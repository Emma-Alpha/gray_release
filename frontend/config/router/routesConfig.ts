import type { IMemo, IRoutesById, RouteItem } from './types';

export function getConfigRoutes(opts: { routes: IRoutesById[] }) {
...
function transformRoute(opts: {
  ...
  if (absPath?.charAt(0) !== '/') {
    ...
    absPath = endsWithStar(parentAbsPath)
      ? parentAbsPath
      : ensureWithSlash(parentAbsPath, absPath as string);
  }
  ...
  if (wrappers?.length) {
    ...
    const layout = opts.route.layout;
    ...
    wrappers.forEach((wrapper: any) => {
      ...
      parentId = id;
      path = endsWithStar(path as string) ? '*' : '';
    });
